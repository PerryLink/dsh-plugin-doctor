...
function collectSourceFiles(ctx) {
  ...
  let mode = 'src'
  if (!files.length && main.startsWith('lib/')) {
    const libFiles = findFiles(repoPath, 'lib', /\.(mjs|js|cjs)$/)
    if (libFiles.length) { files = [...new Set(libFiles)]; mode = 'lib-fallback' }
  }
  if (!files.length) mode = 'none'
  ctx.coverage = { ...(ctx.coverage ?? {}), K: { filesInspected: files.length, mode } }
  return files
}
