...
/**
 * 选项对象逐字段声明（仅为类型面；运行时行为不变）：
 * `cwd`/`logDir`/`shell` 无默认值，若不给 JSDoc，TS 会把形参类型推成默认值 `{}`
 * 的推断结果，于是解构本身与各调用点都会被判「属性不存在」。
 *
 * @param {string} label
 * @param {string} cmd
 * @param {string[]} args
 * @param {{ env?: Record<string, string | undefined>, timeout?: number, cwd?: string, logDir?: string, shell?: boolean }} [options]
 */
